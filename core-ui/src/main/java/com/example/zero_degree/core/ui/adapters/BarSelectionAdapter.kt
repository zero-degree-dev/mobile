...
class BarSelectionAdapter(
    ...
    fun setSelectedBar(bar: Bar?) {
        ...
        selectedBar = bar
        oldSelected?.let { 
            val index = currentList.indexOfFirst { it.id == oldSelected?.id }
            if (index >= 0) notifyItemChanged(index)
        }
        bar?.let { 
            val index = currentList.indexOfFirst { it.id == bar.id }
            if (index >= 0) notifyItemChanged(index)
        }
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BarViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(CoreUiR.layout.item_bar_selection, parent, false)
    ...
    inner class BarViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val tvBarAddress: TextView = itemView.findViewById(CoreUiR.id.tvBarAddress)
        private val tvSelected: TextView = itemView.findViewById(CoreUiR.id.tvSelected)
